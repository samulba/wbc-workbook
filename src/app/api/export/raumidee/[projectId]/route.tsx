...
// ── Static lookup tables (no Lucide imports needed) ───────────────────────────
...
export async function GET(
  ...
  // Fetch project + room + module1_analysis
  ...
  // Moodboard URL (first valid entry)
  ...
  // Build a safe ASCII filename + RFC 5987 encoded version for Unicode chars
  const safeName = project.name
    .replace(/[^\w\s\-äöüÄÖÜß]/g, "")
    .replace(/\s+/g, "-")
    .slice(0, 60);
  const dateSlug = now.toLocaleDateString("de-DE").replace(/\./g, "-");
  const filename = `Raumidee-${safeName}-${dateSlug}.pdf`;
